...
```
ubuntu@ip-10-0-100-207$ sudo apt-get install docker-buildx-plugin
```

플러그인을 설치한 이후 아래와 같이 빌더를 하나 만들어 준다. 이 빌더는 멀티 아키텍처 이미지를 만드는 데 사용된다.  
```
ubuntu@ip-10-0-100-207$ sudo docker buildx create --name multiarch --bootstrap --use

ubuntu@ip-10-0-100-207$ sudo docker buildx inspect multiarch
```


ecr 에 로그인한다.
```
ubuntu@ip-10-0-100-207$ sudo apt install -y awscli

ubuntu@ip-10-0-100-207$ aws ecr get-login-password --region ap-northeast-2 | docker login --username AWS --password-stdin \
...
```
ubuntu@ip-10-0-100-207$ docker buildx build --push \
