...
2. remote 컨텍스트 생성하기
```
ubuntu$ docker context create node-arm --docker "host=tcp://10.0.102.53"
ubuntu$ docker context create node-x86 --docker "host=tcp://10.0.101.4"
...
ubuntu$ docker --context node-arm build --no-cache --force-rm true --rm true \
-t spring-ai -f ./Dockerfile.gitclone
```   
