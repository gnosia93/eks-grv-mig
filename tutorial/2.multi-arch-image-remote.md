...
```
ubuntu$ sudo docker buildx create --name local_remote_builder --node x86-jenk --platform linux/amd64

ubuntu$ sudo docker buildx create --name local_remote_builder --append --node arm-jenk --platform linux/arm64 tcp://10.0.102.180
```
...
ubuntu$ docker buildx use local_remote_builder
ubuntu$ docker buildx inspect --bootstrap
```
